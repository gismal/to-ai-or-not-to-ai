...
class InferenceService:
    ...
    async def process_upload(self, file: UploadFile) -> dict:
        ...
        safe_filename = PurePosixPath(file.filename or "unnamed.png").name
        ...
        if cache_key:
            ...
            if cached_result:
                ...
                parsed_cache["processing_time_ms"] = round(
        ...
        if "UNCERTAIN" in final_prediction.value:
            logger.warning(
                f"UNCERTAIN detected: {file.filename} (Score: {result.confidence})"
            )
