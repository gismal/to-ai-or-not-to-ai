...
def test_predict_invalid_format(client, auth_headers):
    client.app.state.inference_service.process_upload.side_effect = (
        InvalidImageFormatError("bad format")
    )
...
def test_predict_model_error(client, auth_headers):
    client.app.state.inference_service.process_upload.side_effect = ModelInferenceError(
